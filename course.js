...
function renderHello() {
	...
	function renderChild(child) {
		...
		if (child.hasOwnProperty('link')) {
			...
			player = (child.file.hasOwnProperty('player')) ? child.file.player : '';
			out += '<span class="link">';
			out += '<a class="tocLink" href="javascript:void(0);"';
	...
	function renderChildren(children, level) {
		...
		children.forEach( function(child) {
			if (level === 4 && child.hasOwnProperty('file')) {
				child.file.player = 2;
			}
			out += "<li>";
	...
	var level = 0;
    var rendered = renderChildren(playlist.my_tree.collection, level);
    var target = document.getElementById("tocContainer");
